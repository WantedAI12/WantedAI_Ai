...
V1/V2 산업 레지스트리 확장에서는 무취·정보 부족 원료에 이름/SMILES/해시 기반의
향 프로필이 부여될 수 있었다. V11은 이 경로와 맛-냄새 혼입을 차단한다.
기존 확장 원료 기반 높은 점수는 현실적인 향 일치의 근거로 사용하지 않는다.
상세 원인과 재현 경계는 [V11 보고서](benchmarks/ODOR_DATA_INTEGRITY_V11.md)에 기록했다.

3,724개 R&D 원료의 RDKit 구조 계산값을 연결했지만, 분자 기술자 커버리지와
실측 증기압·역치 커버리지는 다르다. `calculated_structure_coverage_percent`는
계산값이 사용된 처방 비중이며 실측 정확도나 인간 후각 정확도를 뜻하지 않는다.
...
트윈 2.4 후보는 brief의 오프닝·하트·드라이다운 목표를 각 시간점에 적용한다.
전체 금지와 구간별 금지를 별도로 보존하며, 지정하지 않은 구간은 전체 프로필을 사용한다.
원료 물성 입력과 농도별 응답 행렬을 재사용하고, 같은 제약 안의 원료 교체안을 기존 안과 비교한다.
이 후보의 점수는 구간별 목표를 포함하므로 구간 구분이 없던 이전 점수와 직접 비교하지 않는다.
...
2026-09-05의 트윈 2.3은 시간별 잔존율을 먼저 OAV에 적용한 후 포화 반응을
계산한다. 반응식은 `S(x) = x^0.55 / (1 + x^0.55)`이며,
`S(OAV(0) * 잔존율)` 뒤에 전달 계수와 혼합 억제를 적용한다.
초기 반응 `S(OAV(0))`에 잔존율을 곱하던 순서 오류를 수정했으며 명목 곡선,
Monte Carlo, 최적화용 물성 계수에 같은 순서를 사용한다.
PhysSim 1.2의 로그 가중치도 `log1p(OAV(0) * 잔존율)`로 계산한다.
이 변경은 수식 일관성 수정이므로 시간 점수와 선택 조향식이 이전 버전과 달라질 수 있다.
