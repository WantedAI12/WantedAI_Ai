...
def main():
    ...
    model = FormulationNetwork(x.shape[1], 450, 292, len(meta["actions"])).to(device)
    ...
    if args.initial:
        ...
        if unexpected or set(missing) - {"emulsion_head.weight", "emulsion_head.bias"}:
    ...
    optimizer = torch.optim.AdamW(
        model.parameters(), lr=0.0002 if args.initial else 0.0008, weight_decay=0.0001
    )
