...
from deploy.runtime_release_v87 import ROOT, verify, registry_path
from deploy.target_runtime_v87 import create_release_app, verify_target_contract

REMOTE_ROOT = '/opt/perfumery/runtime'
LOCAL_BUNDLE = ROOT/'tmp/modal-runtime-v87/release-01'
BUNDLE_SHA256 = 'ef1396157a16d0034d32c202dc4f8007d693441947d9549403774672a2020d65'
WHEEL_REL = '.benchmarks/v87_failed_recovery/package-05-target90/wheel/perfumery_ai_core-1.4.0-py3-none-any.whl'
if modal.is_local():
...
image = (
    ...
    .run_commands(f'python -m deploy.runtime_release_v87 --check-installed {REMOTE_ROOT} --sha256 {BUNDLE_SHA256}')
)
...
def verify_release(snapshot_requests=None):
    ...
    manifest = verify(REMOTE_ROOT, BUNDLE_SHA256)
    checked_app = release_app()
    target_contract = verify_target_contract(checked_app)
    with TestClient(checked_app, raise_server_exceptions=False) as client:
