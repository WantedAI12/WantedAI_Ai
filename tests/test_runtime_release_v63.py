...
from deploy.runtime_release_v63 import RELEASE_ID,WHEEL_SHA256,collect,sha,verify


def test_release_requires_the_exact_selected_profile(tmp_path):
    (tmp_path/'perfumery.local.json').write_text('{}')
    with pytest.raises(ValueError,match='selected V63 profile changed'):collect(tmp_path)
...
def test_deployment_keeps_auth_endpoint_identity_and_no_production_promotion():
    text=(Path(__file__).resolve().parents[1]/'deploy/modal_release_v63.py').read_text()
...
def test_cross_python_worker_uses_importable_class_and_explicit_package_mount():
    ...
    assert ".add_local_python_source('deploy')" in (root/'deploy/modal_release_v63.py').read_text()
