...
- Registry: 29,240 molecules connected / 34 curated active / 637 risk-tier-2 R&D trace candidates
...
29,240개 전체 레지스트리의 조건부 trace 후보를 명시적으로 사용하려면 다음 두
필드를 함께 보냅니다. 이 모드의 결과는 `experimental_registry_candidate`이며
제조·상용 후보가 아닙니다.
...
- `dist/full-registry-activation-v1/`: 현재 배포 Wheel과 release manifest
...
- 실제 원격 확장 처방: `experimental_registry_candidate`, 12 ingredients 중
  registry trace 2, 각 최대 0.05%, manufacturing false
...
공개 descriptor 기반 637개 조건부 trace 후보는 명시적 prototype R&D 모드에만
열리며 독립 안전 승인 또는 공급 가능한 상용 원료로 표현하지 않습니다.
